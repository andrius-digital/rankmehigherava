...
const PTO_TYPE_EMOJI = { vacation: '🏖️', sick: '🤒', personal: '👤', holiday: '🎉', other: '📋' };
...
app.post('/api/pto/notify', async (req, res) => {
  try {
    ...
    if (!member_name || !start_date || !end_date) return res.status(400).json({ error: 'Missing fields' });
...
app.get('/api/pto/reminders', async (req, res) => {
  try {
    ...
    const { data: entries, error } = await sb.from('pto_entries').select('*').eq('start_date', tomorrowStr);
    if (error) return res.status(500).json({ error: error.message });
    if (!entries || entries.length === 0) return res.json({ success: true, reminders_sent: 0, message: 'No PTO starting tomorrow' });
    let sent = 0;
