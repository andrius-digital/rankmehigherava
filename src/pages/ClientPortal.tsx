...
const getClientLogo = (client: any): string | null => {
    // Use Google's favicon service to get logo from website URL
    if (client.website_url) {
        return `https://www.google.com/s2/favicons?domain=${encodeURIComponent(client.website_url)}&sz=128`;
    }

    // For funnel clients without website_url, try to get from notes (funnel domain)
    if (client.notes) {
        try {
            const parsed = JSON.parse(client.notes);

            // Check for live_domain or dummy_domain in funnel data
            const domain = parsed?.live_domain || parsed?.dummy_domain;
            if (domain) {
                return `https://www.google.com/s2/favicons?domain=${encodeURIComponent(domain)}&sz=128`;
            }

            // Fallback: Try to get uploaded logo_files
            let logoFiles = parsed?.logo_files || parsed?.form_submission?.logo_files;

            if (logoFiles) {
                if (Array.isArray(logoFiles) && logoFiles.length > 0) {
                    const firstLogo = logoFiles[0];
                    if (typeof firstLogo === 'string' && firstLogo.startsWith('http')) return firstLogo;
                    if (firstLogo?.url) return firstLogo.url;
                } else if (typeof logoFiles === 'string' && logoFiles.startsWith('http')) {
                    return logoFiles;
                }
            }
        } catch (error) {
